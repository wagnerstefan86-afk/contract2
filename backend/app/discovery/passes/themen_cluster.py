...
class TopicEvidenceRef:
    ...
    def has_deterministic_provenance(self) -> bool:
        """Return True if this ref has at least one deterministic identifier.

        Title-only refs do NOT count as having provenance.
        """
        return self.source_raw_index is not None or self.source_fingerprint is not None
...
class LinkageStats:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "refs_missing_provenance": self.refs_missing_provenance,
...
def resolve_topic_fundstellen(
    ...
    for ci, cluster in enumerate(clusters):
        for ref in cluster.evidence_refs:
            ...
            if not ref.has_deterministic_provenance():
                stats.refs_missing_provenance += 1
